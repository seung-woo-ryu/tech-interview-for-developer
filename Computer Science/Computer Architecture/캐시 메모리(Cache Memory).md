...
***멀티 코어 프로세서의 캐시 일관성 *** : 멀티 코어가 등장하면서 각 코어마다 캐시를 장착하게 되었다. 이러한 시스템에서 캐시 간 일관성이 중요하다. 한 코어에서 캐시 라인이 업데이되면, 일관성을 위해 다른 코어의 캐시라인은 invalidate되는데 이러한 캐시 데이터에 접근하면 캐시 미스가 일어난다.
